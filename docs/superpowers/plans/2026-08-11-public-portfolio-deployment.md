...
**Architecture:** Vercel Web 从 monorepo 构建 `apps/web`，同时把权威案例页与证据同步到 `public/`；另一个 Vercel Project 以 `apps/api` 为根目录，把 `app.main:app` 部署为 FastAPI Function。前端通过构建时环境变量访问 API，API 通过严格的 `ALLOWED_ORIGINS` 接受唯一生产前端 origin。

**Tech Stack:** Next.js 16、React 19、FastAPI、Vercel Python Runtime、pnpm、uv、Vercel、GitHub CLI、Playwright。
...
- API 使用单一 FastAPI Function；明确披露进程内会话会因实例替换、扩缩容或部署而丢失。
- 所有新行为先 RED、再最小 GREEN；配置文件用静态验证和真实构建证明。

---

### Task 1: 严格生产 CORS 与云端运行入口
